...
15. on photos add a animation of sea highway son in it a Mercedes Benz s class car so if we drag it up so a color as fill and zoom in and if we move it down so zoom out and it place under the full screen icon and further this full screen icon and this icon make under the right side carousel arrow as problem is full screen icon is mixing with it and further under a one icon make name as many things and a treasure box so when we click so it opens and then these two options as full screen and this zoom icon will appear so which we click that further works and then also a cross exit icon make on there card to close and also when in video if we click these controls go away also ( so in this way less clutter on the screen and also user know how it is working )
16. on mobile the country flag is showing in the phone number country code but on laptop not showing
17. add a onload and onerror in my whole website for the images, videos, text, short videos and even for whole website and make it as like if full website is loading so show a blurr effect on my theme web page and as it loading so it is going away and If error any then show in red text in big font what is error that and under it how to solve the instruction and if it need just retry then a retry button also there and for images, videos, short videos and texts show a blurr effect when they are loading and it goes away when it is loaded and further if error then show them error in a red text big font and the instruction as how to solve it and if retry desire then a retry button and in-short I am making my own style this way and which make users understand like loading is happening so they can wait and further to make there experience more understanding show the live detail if possible on loading and on error so they know what problem is for example in case of slow internet or internet is gones show them internet is not as it's speed so user know it's data is end or any problem is etcs 
18. and make add attribute of async for external script of js for more good performance of my web and user better experience like for example in case of loading like on window means when web someone is opening so if loading so show them loading as I tell you in blurr and it going away and make the functionality like button disable till they are not functional totally as you know the page appear fastly in async attribute in script so it is not comedy fully yet so if user click on button or anything which is not totally functional then user confuse for that make them disable and show them a wait... so they wait etcs and then they can use for more good etcs 
