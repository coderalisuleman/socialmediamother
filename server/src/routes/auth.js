...
import { cleanLoginIdentifier, cleanUsername } from '../utils/normalize.js';
...
authRouter.post('/signup', sensitiveLimit, asyncHandler(async (req, res) => {
  const {
    fullName, password, confirmPassword, gender,
    method, otpVerificationToken
  } = req.body || {};
  ...
  assert(['email', 'phone'].includes(method), 422, 'Create your account with a phone number or email', 'INVALID_SIGNUP_METHOD');
  assert(['female', 'male', 'transgender', 'other'].includes(gender), 422, 'Choose a valid person type', 'INVALID_GENDER');
...
authRouter.post('/login', sensitiveLimit, asyncHandler(async (req, res) => {
  ...
  const user = await findUserByIdentifier(normalizedIdentifier, { includePassword: true });
  if (!user || !await bcrypt.compare(password, user.passwordHash)) {
    throw new AppError(401, 'Phone/email or password is incorrect', 'INVALID_CREDENTIALS');
  }
